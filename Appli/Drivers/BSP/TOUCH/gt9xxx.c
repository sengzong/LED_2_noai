...
uint8_t gt9xxx_scan(uint8_t mode)
{
    ...
    if ((t % 4) == 0 || t < 4)  /* 每4次读一次(原t%10),让高亮跟随更跟手 */
    {
        ...
        if ((mode & 0XF) && ((mode & 0XF) <= g_gt_tnum))
        {
            ...
            for (i = 0; i < g_gt_tnum; i++)
            {
                if (tp_dev.sta & (1 << i))  /* 触摸有效? */
                {
                    ...
                    if (rgblcddev.id == 0X4384)
                    {
                        /* 4.3in 800x480 RGB panel, portrait display 480x800:
                         * GT911 raw X (buf[3..2]) = panel long edge  -> display Y
                         * GT911 raw Y (buf[1..0]) = panel short edge -> display X */
                        uint16_t rx = ((uint16_t)buf[3] << 8) + buf[2];
                        uint16_t ry = ((uint16_t)buf[1] << 8) + buf[0];
                        tp_dev.x[i] = (rgblcddev.width - 1) - rx;   /* horizontal, reversed */
                        tp_dev.y[i] = ry;                            /* vertical, direct */                    }
                    else if (rgblcddev.id == 0X5510 || rgblcddev.id == 0X9806 || rgblcddev.id == 0X7796)     /* 4.3寸800*480(MCU屏与RGB屏) 和 3.5寸480*320 MCU屏 */
                    {
                        if (tp_dev.touchtype & 0X01)    /* 横屏 */
                        {
                            tp_dev.x[i] = rgblcddev.width - (((uint16_t)buf[3] << 8) + buf[2]);
            ...
            if (tp_dev.x[0] > rgblcddev.width || tp_dev.y[0] > rgblcddev.height)  /* 非法数据(坐标超出了) */
            {
