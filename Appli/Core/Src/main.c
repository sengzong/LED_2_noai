...
#include "../../Drivers/BSP/HyperRAM/hyperram.h"
#include "../../Drivers/BSP/SYS/sys.h"
#include "model_select.h"
#include "../../Drivers/BSP/YT8512C/yt8512c.h"
#include "app_azure_rtos.h"             /* ThreadX: tx_application_define + tx_kernel_enter */
...
volatile uint32_t g_hyp = 0xFF;          /* 位0=init失败, 位1=mmap失败; 0=成功 */
volatile uint8_t g_rtos_started = 0;     /* ThreadX 内核启动标志(SysTick_Handler 用) */
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* D-Cache: 禁用。帧缓冲在内部 SRAM, CPU 绘制 OTA 页, 避免缓存残留致 LTDC 读到旧内容 */
  SCB_DisableDCache();
  ...
  /* MPU Configuration--------------------------------------------------------
  MPU_Config();   /* 注释: 参考工程 352_IMX335 不调用 MPU_Config。
                     MPU_PRIVILEGED_DEFAULT 背景区可能把未配置的 PSRAM(0x90000000)
                     读写变无效(读到 0x5555) -> 帧缓冲/相机缓冲全坏 -> 显示蓝/无帧 */
  HAL_Init();

  /* USER CODE BEGIN Init */
#ifdef DEBUG
  sys_clock_config_debug();               /* debug 直跑(无 FSBL)时必须自己配 PLL1 等时钟 */
#endif
  /* USER CODE END Init */
...
#ifdef DEBUG
  MX_XSPI1_Init();                        /* 仅 Debug(SRAM 直跑/无 FSBL)才由 App 自己 init PSRAM */
  g_hyp = ((uint32_t)HyperRAM_Init(&HyperRAMObject, &hxspi1) << 0) |
          ((uint32_t)HyperRAM_EnableMemoryMappedMode(&HyperRAMObject) << 1);
#endif
  ...
  MX_DMA2D_Init();
  MX_LTDC_Init();
  /* MX_XSPI1_Init();  PSRAM 由 FSBL 映射，Release 不再 init */
  /* MX_XSPI2_Init();  NOR 正承载 XIP 代码，不重配 */
  /* MX_ETH1_Init();   在 USER CODE 2(rgblcd 之后)执行 */
  MX_I2C2_Init();                     /* 触摸用硬件 I2C2 */
  SystemIsolation_Config();
  /* USER CODE BEGIN 2 */
#ifdef DEBUG
  if (g_hyp)
  {
    /* Debug(SRAM)下 PSRAM 未就绪提示: bit1→LED0 常亮 = mmap 失败; bit0→LED1 常亮 = init 失败 */
    HAL_GPIO_WritePin(GPIOG, GPIO_PIN_10, (g_hyp & 0x2) ? GPIO_PIN_RESET : GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOE, GPIO_PIN_10, (g_hyp & 0x1) ? GPIO_PIN_RESET : GPIO_PIN_SET);
    while (1) { }
  }
#else
  /* Release/FSBL 引导：PSRAM 已由 FSBL 映射到 0x90000000，App 直接用，不再 init */
#endif
  rgblcd_init();                          /* 初始化RGB LCD（提前，尽早覆盖上电花屏） */
  rgblcd_clear(WHITE);                    /* 立刻刷白屏 */
  SCB_CleanDCache();                      /* 保险调用 */
  uart_init(115200);                      /* printf over USART1 */
  printf("STM32N647 LED_2 - OTA RTOS boot  [FW-TARGET v3]\r\n");
  printf("OTA probe: 0x%08X (EXPECT 0x314F5441 = .ota_ram SRAM agent OK)\r\n", (unsigned)ota_probe_run());
  ota_dbg_ladder();               /* 真机分步 OTA 测试: 级别见 app_netxduo.c OTA_BOOT_LADDER_LEVEL */
  printf("ETH MAC init...\r\n");
  MX_ETH1_Init();                         /* ETH MAC 初始化 */
  printf("Ethernet PHY init...\r\n");
  yt8512c_init();                         /* Ethernet PHY (YT8512C) bring-up */
  printf("ETH1 ready (HW)\r\n");
  printf("ThreadX booting...\r\n");
  g_rtos_started = 1;                     /* 置位后 SysTick_Handler 才调 _tx_timer_interrupt() */
  tx_kernel_enter();                      /* 永不返回；UI/OTA(model_select) 作为线程运行 */
...
static void MX_XSPI2_Init(void)
{
  ...
  hxspi2.Init.MemorySize = HAL_XSPI_SIZE_256B;
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOG, GPIO_PIN_6|GPIO_PIN_4|GPIO_PIN_10, GPIO_PIN_RESET);
...
void Error_Handler(void)
{
  ...
  /* Error visible on board: LED0(PG10) fast-blink + LED1(PE10) solid ON */
  {
    volatile uint32_t i;

    HAL_GPIO_WritePin(GPIOE, GPIO_PIN_10, GPIO_PIN_RESET);   /* LED1 常亮 */
    for (;;)
    {
      HAL_GPIO_WritePin(GPIOG, GPIO_PIN_10, GPIO_PIN_RESET); /* LED0 快闪 */
